...
export async function getTemplateById(id) {
  const [rows] = await pool.query(
    `SELECT * FROM route_templates WHERE id = ? LIMIT 1`,
    [id],
  );
...
export async function getTemplateWithDetails(id) {
  ...
  const [schedules] = await pool.query(
    `SELECT * FROM operating_schedules WHERE route_template_id = ?`,
    [id],
  );
...
export async function createTemplate(groupId, data) {
  ...
  await pool.query(
    `INSERT INTO route_templates
       (id, group_id, label, reason, points, description, is_loop, fare_price, schedule_start, schedule_end, frequency)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      id,
      groupId,
...
export async function updateTemplate(id, data) {
  await pool.query(
    `UPDATE route_templates
     SET label = ?, reason = ?, points = ?, description = ?, is_loop = ?, fare_price = ?, schedule_start = ?, schedule_end = ?, frequency = ?
     WHERE id = ?`,
    [
      data.label,
      data.reason ?? null,
...
export async function duplicateTemplate(sourceRouteId, { label, reason }) {
  ...
  try {
   ...
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    const [fareSegments] = await conn.query(
      `SELECT * FROM fare_segments WHERE route_template_id = ?`,
      [sourceRouteId],
    );
...
export async function archiveOrDeleteTemplate(id) {
  ...
  if (rows[0].count > 0) {
    await pool.query(
      `UPDATE route_templates SET is_archived = TRUE WHERE id = ?`,
      [id],
    );
    return "archived";
  }
