...
import { notifyFavoritesOfRouteChange } from "./routeChangeNotifications.js";
...
export async function getGroupById(id) {
  const [rows] = await pool.query(
    `SELECT * FROM route_groups WHERE id = ? LIMIT 1`,
    [id],
  );
  return rows[0] ?? null;
}
...
export async function updateGroup(
  id,
  { name, description, transportType, isActive },
) {
  await pool.query(
...
export async function setDefaultTemplate(groupId, routeTemplateId) {
  ...
  if (rows.length === 0)
    throw new Error("La plantilla no pertenece a este grupo");

  await pool.query(
    `UPDATE route_groups SET default_route_id = ? WHERE id = ?`,
    [routeTemplateId, groupId],
  );
}
...
export async function setActiveTemplate(
  groupId,
  targetRouteId,
  { triggeredBy, ruleId = null, reason = null, performedByUserId = null },
) {
  ...
  try {
    ...
    const [groupRows] = await conn.query(
      `SELECT * FROM route_groups WHERE id = ? FOR UPDATE`,
      [groupId],
    );
    ...
    if (group.active_route_id === targetRouteId) {
      await conn.commit();
      return group; // no-op, ya estaba activa: no se notifica nada
    }

    const [templateRows] = await conn.query(
      `SELECT id FROM route_templates WHERE id = ? AND group_id = ?`,
    ...
    await conn.query(
      `UPDATE route_groups SET active_route_id = ? WHERE id = ?`,
      [targetRouteId, groupId],
    );
    ...
    // El cambio ya quedó confirmado en DB; notificar es best-effort y no
    // debe poder revertir ni afectar el resultado de esta función.
    await notifyFavoritesOfRouteChange({ groupId, triggeredBy, reason });
...
export async function revertToDefault(groupId, { reason, userId } = {}) {
  ...
  if (!group?.default_route_id)
    throw new Error("Este grupo no tiene plantilla default definida");
...
export async function lockManual(groupId, { routeTemplateId, reason, userId }) {
  ...
  try {
    ...
    await notifyFavoritesOfRouteChange({
      groupId,
      triggeredBy: "MANUAL_LOCK",
      reason,
    });
  } catch (err) {
    await conn.rollback();
...
export async function unlockAndRevertToDefault(
  groupId,
  { reason, userId } = {},
) {
  const group = await getGroupById(groupId);
...
export async function logConflictWarning(
  groupId,
  { newRouteId, ruleId, reason },
) {
  const group = await getGroupById(groupId);
  await pool.query(
...
export async function listActiveGroupsWithRules() {
  const [groups] = await pool.query(
    `SELECT * FROM route_groups WHERE is_active = TRUE`,
  );
